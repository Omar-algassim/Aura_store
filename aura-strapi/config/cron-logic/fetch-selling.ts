...
const fetchSelling = async ( db: knex.Knex, orderBy: string) => {
  ...
  const selling = await db('products').select('id').orderBy('sales', orderBy).limit(10);
...
const fetchBestSelling = async (db: knex.Knex) => {
  const {selling:bestSelling, periodStart, periodEnd} = await fetchSelling(db, 'desc');
  const bestSellingRecord = await db('best_selling').insert(bestSelling.map(({id}) => ({product_id: id, period_start: periodStart.toISOString(), period_end: periodEnd.toISOString()})));
...
const fetchSteadySelling = async (db: knex.Knex) => {
  const {selling:steadySelling, periodStart, periodEnd} = await fetchSelling(db, 'asc');
  const steadySellingRecord = await db('steady_selling').insert(steadySelling.map(({id}) => ({product_id: id, period_start: periodStart.toISOString(), period_end: periodEnd.toISOString()})));
