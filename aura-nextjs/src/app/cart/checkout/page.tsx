...
function CheckoutPage() {
  ...
  const deliveryFees = (settings.delivery_fees || 60) * settings.exchange_fees;

  const total = cart?.total_pay
    ? deliveryFees + cart?.total_pay
    : deliveryFees;
  const bankAccountNumber = settings.bank_account_number || '2006899';
        ...
        <div className='w-full  bg-surface rounded-xl flex flex-col items-start justify-between px-4 py-12 gap-5'>
          ...
          <p className='text-sm md:text-xl font-[500] capitalize'>
            ...
            <Link
              ...
              className='italic text-primary-dark border-b'>
              {' واتساب '}
            </Link>
            لمساعدتك في إتمام عملية الدفع
          </p>
        </div>
