...
import { newCategoryAction, updateCategory, createCategory } from "@/utils/services/dashboard/category";
...
export default function CategoryForm(props: categoryProps) {
    ...
    async function handleSave(
      ...
      if (editMode && category) {
        const { error, data: updatedData } = await updateCategory(category.documentId, jwt, data);
        if (error) {
          return {
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
              <div className="px-2 pr-14">
                  ...
                  <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                   {editMode? `edit ${category?.title}` : " register new category in store."}
                  </p>
                </div>
                <form
                action={action}
                  ...
                  <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
                    <div>
                      <h5 className="mb-5 text-lg font-medium text-gray-800 dark:text-white/90 lg:mb-6">
                        ...
                        <div>
                          <Label>Title *</Label>
                          <Input name="title" defaultValue={category?.title} placeholder="the name of product" type="text"/>
                {titleError.length > 0 ? (
                  ...
                  <span className="flex-1 text-primary-dark text-xs text-right font-[400] font-alex max-w-[200px] h-8 text-wrap"></span>
                        ...
                        <div>
                          ...
                          <Input name="priority" defaultValue={category?.priority} placeholder="Number represent the sort of category in the list" type="number"/>
