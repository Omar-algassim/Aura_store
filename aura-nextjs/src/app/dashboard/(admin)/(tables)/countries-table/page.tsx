...
import { NewRegionForm } from '@/app/dashboard/(admin)/(forms)/new-country/NewRegionForm';
...
export default function CountryList() {
  ...
  const [toDelete, setToDelete] = React.useState<Country | undefined>(
    undefined
  );
  ...
  async function handleDeleteRegion() {
    ...
    try {
      ...
      toast({
        ...
        title: 'Error',
        description:
          error.message || 'An error occurred while deleting the country.',
      });
      return;
    }
  }
    ...
    <div className='overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6'>
      <div className='flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-end'>
        <div className='flex items-center gap-3'>
          <button
            ...
            className='inline-flex items-center cursor-pointer gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200'>
            <PlusIcon />
            New Country
          </button>
        </div>
      ...
      <Modal
        ...
        <NewRegionForm
          editMode={true}
          country={edit}
          toggleEditModal={() => toggleEditModal(undefined)}
        />
      </Modal>
      <Modal
        isOpen={alerting}
      ...
      <Modal
        ...
        <NewRegionForm
          editMode={false}
          toggleEditModal={newCountryWindow}
        />
      </Modal>
    </div>
  );
