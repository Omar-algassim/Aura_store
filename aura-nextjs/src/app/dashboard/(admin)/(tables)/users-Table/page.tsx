...
if (edit?.blocked && edit?.id) {
      unblockUser(cookie.get("jwt") || "", edit.id)
        .then((response) => {
          if (response.error) {
      ...
      blockUser(cookie.get("jwt") || "", edit.id)
        .then((response) => {
          if (response.error) {
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
                        ...
                        {users.map((user) => ( user.id !== me?.id && (
                          <TableRow key={user.id}>
                            ...
                            <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <Badge
                                size="sm"
                                color={user.blocked ? "error" : user.confirmed ? "success" : "warning"}
                              >
                                {user.blocked ? "Blocked" : user.confirmed ? "Confirmed" : "need to confirm"}
                              </Badge>
                            </TableCell>
                            <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                              <button onClick={() => toggleModal(user)} className={`px-4 py-2 text-sm font-medium text-white ${user.blocked ? "bg-green-600 rounded-md hover:bg-green-700" : "bg-red-600 rounded-md hover:bg-red-700"}`}>
        ...
        <Modal isOpen={isOpen} onClose={() => setIsOpen(!isOpen)} className="w-full h-[340px] justify-center justify-items-center max-w-md">
            <div className="pt-26 p-6 justify-items-center justify-center items-center">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white/90">
               {edit?.blocked ? "are you sure you want to unblock user?" : `are you sure you want to block user?`}
              </h2>
              <p className="mt-2 text-gray-500 dark:text-gray-400">
                {edit?.blocked ? "unblocking a user will allow them from accessing their account and using the application." :"Blocking a user will prevent them from accessing their account and using the application."}
              </p>
              <div className="mt-4 gap-4 flex justify-end">
                <button
                  className={`px-4 py-2 text-sm font-medium text-white rounded-2xl ${edit?.blocked ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}`}
