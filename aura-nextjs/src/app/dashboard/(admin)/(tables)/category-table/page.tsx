...
import { CategoryForm } from '@/app/dashboard/(admin)/(forms)/new-category/CategoryForm';
...
export default function CategoryList() {
  ...
  async function handleDeleteCategory() {
    ...
    try {
      ...
      setCategories(
        categories.filter((category) => category.id !== toDelete.id)
      );
      setAlerting(false);
  ...
  return (
    <div className='overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6'>
      ...
      <Modal
        ...
        onClose={newCategoryWindow}
        className='max-w-[700px] m-4'>
        <CategoryForm
          editMode={false}
          category={undefined}
          toggleEditModal={newCategoryWindow}
        />
      </Modal>
    </div>
  );
