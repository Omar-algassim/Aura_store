...
export async function updateOrderStatus(
  ...
  try {
    const {error, data} = await apiClient.updateOrderStatus(jwt, orderId, status);
    if (error) {
      return {
        message: error.message,
        type: "server error",
...
export async function SendWhatsappMessage(
  phone: string,
  body?:{template_id?: string, templateVariables?: Record<string, string>, template_header_media_url?: string,}
): Promise<{
  message: string;
  type?: string;
  data?: any;
  ...
  const WHATS_URL = process.env.WHATS_URL || "https://app.xpressbot.org/api/v1/whatsapp/send/template";
  if (!WHATS_TOKEN || !WHATS_SENDER || !WHATS_URL) {
    return {
      message: "WhatsApp configuration is missing",
      type: "error",
      data: null,
      error: new Error("Missing WhatsApp configuration"),
    };
  }
