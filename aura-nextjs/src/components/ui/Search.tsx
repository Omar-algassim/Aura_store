...
const Result: React.FC<ResultProps> = ({ Result, word }) => {
  ...
  if (!Result) {
    ...
    return (
      <div className='no-scrollbar w-full flex gap-1 bg-blue_shade my-[2px]'>
        {/* product content */}
        <div className='w-full flex flex-col justify-start'>
          <Link
            ...
            <Highlighter
              autoEscape={true}
              highlightClassName={
                'bg-primary text-white rounded-[5px] p-[2px]'
              }
            ...
            <div className='flex gap-x-2 pt-2'>
              <p
                className={`${Result?.discount && 'opacity-75 line-through'}`}>
                {Result.price} SDG
              </p>
              {Result?.discount && (
        ...
        {/* product image */}
        <div className='w-[100px] h-[100px] tablet:w-[120px] tablet:h-[120px] relative'>
          <Image
...
export function Search() {
  ...
  return (
    <>
      <div className='flex flex-col justify-center items-center tablet:relative z-30'>
        <div className='flex items-center rounded-lg justify-between p-[10px] tablet:p-[24px] w-full max-w-[495px] h-[54px] tablet:h-[78px] bg-blue_shade has-focus:ring-2 has-focus:ring-primary'>
          <InputComponent
            name='search'
            type='text'
            value={key}
        ...
        {/* Search Results */}
        <div
          className='absolute left-0 top-full tablet:left-1/2 tablet:transform tablet:-translate-x-1/2 max-h-[430px] w-full max-w-[512px] m-0 mt-2 overflow-x-hidden rounded-xl flex flex-col gap-1 justify-start items-center backdrop-blur-2xl bg-blue_shade/5 peer-focus:animate-scaleIn peer-placeholder-shown:animate-scaleOut'
          id='result-container'
          onClick={() => {
            setKey('');
            setResults([]);
